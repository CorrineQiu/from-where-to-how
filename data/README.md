...
| [RELEASE_REVIEW.md](RELEASE_REVIEW.md) | Metadata validation, file checksums, and annotation export requirements | Annotation validation notes |
...
Final Coherent4D take-level split manifests and portable sample indices must be generated from the exact filtered dataset snapshot used for the reported experiments. FIction's original take lists and absolute feature paths are not substitutes for these files.
